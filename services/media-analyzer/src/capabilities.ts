...
export async function analyzerCapabilities():Promise<Record<string,unknown>>{
 ...
 const value={version:'analyzer-capabilities-1',checkedAt:new Date().toISOString(),decoding:{ffmpeg:Boolean(decoders),office:Boolean(office),pdf:Boolean(pdf),ocr:Boolean(ocr),codes:Boolean(codes)},
