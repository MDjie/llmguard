...
def detect_with_timeout(text, direction="input", timeout=REQUEST_TIMEOUT):
    """调用检测API，使用线程级超时确保不会卡住"""
    result_holder = [None]
    error_holder = [None]
    ...
    def _detect():
        try:
            ...
            with urllib.request.urlopen(req, timeout=timeout) as resp:
                result_holder[0] = json.loads(resp.read().decode("utf-8"))
        except Exception as e:
            error_holder[0] = str(e)
    
    thread = threading.Thread(target=_detect, daemon=True)
    thread.start()
    thread.join(timeout=timeout + 5)  # 给5秒余量
    
    if thread.is_alive():
        return {"success": False, "error": f"TIMEOUT after {timeout}s (thread still running)"}
    
    if error_holder[0]:
        if "timed out" in error_holder[0].lower():
    ...
    return {"success": False, "error": "Unknown error - no result"}
...
def restart_guardllm():
    ...
    for attempt in range(5):
        result = detect_with_timeout("health check", timeout=30)
...
def main():
    ...
    for i, row in enumerate(rows):
        ...
        result = detect_with_timeout(text)
