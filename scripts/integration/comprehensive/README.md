...
执行顺序：初始化与全部迁移 → 合成账号 → 解析器与应用构建 → 单元测试 → 派生聊天控制面与清理崩溃恢复 → 标准资产/清理与派生 RAG → 9 个 Worker → schema/事务/轨迹 → 45 格式夹具 → 页面和业务流程 → 原文与反馈审批 → 安全边界 → 上传与解析终态 → 大文件分层证据 → 180 天查询 → 独立主体注销 → 故障恢复 → 发布门槛 → 清理与汇总。

派生聊天测试使用隔离签名和合成路由，验证完整请求、最终模型 wire 加密归档与发送前撤销，但不调用真实上游模型。清理恢复测试只删除本轮新建合成对象，记录真实子进程中断和重启；其租约/TTL 边界使用受控时间，不代表长期运行或生产清理已获准。上述直接认领任务的测试必须早于后台 Worker。
