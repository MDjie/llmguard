...
async function main(){
 ...
 async function session(credentials:z.infer<typeof credential>){const stateFile=out+(credentials.username===fixture.username?'/browser-state.private.json':credentials.username===fixture.other.username?'/browser-other.private.json':'/browser-foreign.private.json');const reuse=existsSync(stateFile);const api=await request.newContext({baseURL:fixture.baseURL,...(reuse?{storageState:stateFile}:{})});sessions.push(api);if(!reuse){const login=await api.post('/api/auth/login',{data:{username:credentials.username,password:credentials.password}});if(login.status()!==200)throw new Error('LOGIN_FAILED_'+login.status());await api.storageState({path:stateFile});}else if((await api.get('/api/auth/me')).status()!==200)throw new Error('SAVED_SESSION_EXPIRED');const csrf=(await api.storageState()).cookies.find(item=>item.name==='csrf-token')?.value;if(!csrf)throw new Error('CSRF_MISSING');return {api,headers:{'x-csrf-token':csrf,origin:fixture.baseURL}};}
 try{
