...
const reuse=existsSync(out+'/browser-state.private.json');
const api=await request.newContext({baseURL:fixture.baseURL,...(reuse?{storageState:out+'/browser-state.private.json'}:{})});
if(!reuse){const login=await api.post('/api/auth/login',{data:{username:fixture.username,password:fixture.password}});if(login.status()!==200)throw new Error('LOGIN_FAILED_'+login.status());}
if((await api.get('/api/auth/me')).status()!==200)throw new Error('SESSION_REQUIRED');
...
async function upload(name:string,bytes:Buffer,mime:string,kind:string,expected:string,metadata:Record<string,unknown>={},declaredHash?:string){
 ...
 for(let attempt=0;attempt<30;attempt++){const response=await api.get('/api/v1/guard/artifacts/'+artifact.id);if(response.status()!==200)throw new Error('GET_'+name+'_'+response.status());const value=z.object({data:z.object({state:z.string(),failureCode:z.string().nullable(),detectedMediaType:z.string().nullable()})}).parse(await response.json()).data;if(['accepted','failed','quarantined'].includes(value.state)){results.push({artifactId:artifact.id,name,state:value.state,expected,code:value.failureCode,detectedType:value.detectedMediaType,pass:value.state===expected});return;}await new Promise(resolve=>setTimeout(resolve,1000));}throw new Error('VERIFICATION_TIMEOUT_'+name);
}
try{
