import { jointEvidenceProfileSchema } from '../../src/lib/multimodal/joint-evidence-judge';
...
async function main(){
 ...
 if(args.help){console.log('prepare-glm-joint-profile --profile <existing-scoped-profile.json> --out <new.json> [--model glm-5.3|zai-org/GLM-5.3]. Preserves endpoint and secret reference, clears old quality approvals, writes a disabled SHADOW draft.');return;}
 const previous=judgeProfileSchema.parse(await jsonFile(required(args.profile,'profile')));
 const draft={...previous,revision:previous.revision+1,displayName:'GLM-5.3 joint evidence',enabled:false,mode:'SHADOW',
  modelId:args.model??'glm-5.3',modelRevision:null,weightsSha256:null,mutableAlias:true,backendKind:'chat_judge',
 ...
 console.log('Disabled GLM-5.3 SHADOW profile created; previous model qualifications removed. No model call or publication.');
}
