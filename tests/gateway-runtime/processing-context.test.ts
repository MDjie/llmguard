...
it('accepts an archived prepared request without treating it as guard segments',()=>{
 ...
 expect(readProcessingContext({id:'test',sessionSnapshot:[]} as unknown as Parameters<typeof readProcessingContext>[0])).toEqual({inputSegments:[],nativeExecution:false,derivedExecution:false});
});
