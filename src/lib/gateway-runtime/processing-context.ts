...
interface ProcessingContext { inputSegments: ContentSegment[]; nativeExecution: boolean; derivedExecution: boolean; memory?: SecureMemorySnapshot }
export function readProcessingContext(row: typeof gatewayRequests.$inferSelect): ProcessingContext {
  ...
  return { inputSegments: parsed.inputSegments, nativeExecution: Boolean(parsed.artifactContextProof?.manifest.nativeExecution), derivedExecution: Boolean(parsed.artifactContextProof?.manifest.derivedExecution), ...(parsed.memory ? { memory: parsed.memory as SecureMemorySnapshot } : {}) };
}
