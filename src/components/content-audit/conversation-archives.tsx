...
export default function ConversationArchives({ requestId }: { readonly requestId: string }) {
  const [days, setDays] = useState('180'), [requestFilter, setRequestFilter] = useState(requestId), [query, setQuery] = useState(''), [list, setList] = useState<ArchiveList | null>(null);
  ...
  useEffect(() => { setRequestFilter(requestId); }, [requestId]);
  ...
  return <div className="space-y-4">
    <PageHeader title="归档内容版本" description="查询最近 180 天的接收、模型输入、原始输出和释放版本，并核对归档缺口" actions={<Button variant="outline" disabled={loading} onClick={() => refresh()}>刷新</Button>} />
