...
export function ProviderApprovals({userId}:{userId:string}){
  ...
  return <Card><CardHeader><CardTitle>模型上线审批</CardTitle></CardHeader><CardContent className="space-y-4">
    <p className="text-sm text-muted-foreground">{implementationAdmin?'实施测试模式：可核对并批准自己提交的模型上线申请。':'配置或凭据变更后模型保持停用，审批通过才可上线。'}申请绑定具体配置和版本。</p>
    ...
    {rows.map(row=><div key={row.id} className="space-y-2 rounded-md border p-4 text-sm"><p>{row.reason} · {row.status}</p>
      ...
      {row.status==='pending'&&(row.requesterId!==userId||implementationAdmin)&&can('policy:approve')&&<div className="flex gap-2"><Button disabled={busy} onClick={()=>void act('PATCH',{id:row.id,decision:'approve'})}>批准上线</Button><Button variant="outline" disabled={busy} onClick={()=>void act('PATCH',{id:row.id,decision:'reject'})}>拒绝</Button></div>}
    </div>)}
