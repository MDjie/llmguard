...
export const POST=withApiSecurity({permission:'guard:use',bodySchema:schema,responseSchema:jsonObjectResponseSchema,maxBodyBytes:256*1024,auditEvent:'chat.attachments.prepare',rateLimitPolicy:{id:'chat-attachments',windowMs:60000,maxRequests:30,scope:'principal'}},async({body,principal,request})=>{
 ...
 if(ordered.every(row=>row.kind==='TEXT') && ordered.reduce((sum,row)=>sum+(row.verifiedSize??0),0)<=1048576)return Response.json({success:true,data:{mode:'TEXT',references}});
 ...
 // Bind the complete business request before analysis; a summary cannot authorize a different conversation.
 ...
 context.fill(0);
 if(!native){
  const submitted=await submitGuardJob({scope,ownerId,artifactId:ordered[0].id,sourceArtifactIds:body.artifactIds,contextArtifactId:stored.id,taskPurpose:body.messages.findLast(message=>message.role==='user')?.content.slice(0,4096),bundleId:binding.activeBundleId,jobType:'intake',idempotencyKey:body.idempotencyKey,maxAttempts:2});
  return Response.json({success:true,data:{mode:'DERIVED',jobId:submitted.job.id,references:references.map(ref=>({...ref,jobId:submitted.job.id,representation:'TEXT_PROJECTION'})),releaseReason:'CHAT_DERIVED_ROUTE_QUALIFICATION_REQUIRED'}},{status:202});
 }
