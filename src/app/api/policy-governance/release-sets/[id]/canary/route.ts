...
export const POST = withApiSecurity({
  ...
  auditEvent: 'policy.dictionary-set.canary',
  rateLimitPolicy: {id:'dictionary-set-canary',windowMs:60_000,maxRequests:20,scope:'application'},
  ...
  try {
    return Response.json({success:true,data:await operateDictionaryReleaseSet(requireTenantContext(principal),
      principal!.subject,id,'canary',body)});
  } catch(error) {
