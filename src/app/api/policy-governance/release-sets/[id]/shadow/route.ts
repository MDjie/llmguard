...
export const POST = withApiSecurity({
  ...
  auditEvent: 'policy.dictionary-set.shadow',
  rateLimitPolicy: {id:'dictionary-set-shadow',windowMs:60_000,maxRequests:20,scope:'application'},
  ...
  try {
    return Response.json({success:true,data:await operateDictionaryReleaseSet(requireTenantContext(principal),
      principal!.subject,id,'shadow',body)});
  } catch(error) {
