...
export const POST = withApiSecurity({
  ...
  auditEvent: 'policy.dictionary-set.activate',
  rateLimitPolicy: {id:'dictionary-set-activate',windowMs:60_000,maxRequests:20,scope:'application'},
  ...
  try {
    return Response.json({success:true,data:await operateDictionaryReleaseSet(requireTenantContext(principal),
      principal!.subject,id,'activate',body)});
  } catch(error) {
