...
export const POST = withApiSecurity({
  ...
  auditEvent: 'policy.dictionary-set.rollback',
  rateLimitPolicy: {id:'dictionary-set-rollback',windowMs:60_000,maxRequests:20,scope:'application'},
  ...
  try {
    return Response.json({success:true,data:await operateDictionaryReleaseSet(requireTenantContext(principal),
      principal!.subject,id,'rollback',body)});
  } catch(error) {
