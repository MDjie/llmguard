...
import { activateDictionaryRelease, PolicyGovernanceOperationError } from '@/lib/policy-governance';
...
export const POST = withApiSecurity(
  {
    ...
    auditEvent: 'policy.dictionary.activate',
    rateLimitPolicy: { id: 'policy-dictionary-activate', windowMs: 60_000, maxRequests: 20, scope: 'application' },
  },
  async ({ routeContext, body, principal }) => {
    const { id } = await (routeContext as { params: Promise<{ id: string }> }).params;
    try {
      return Response.json({ success: true, data: await activateDictionaryRelease(requireTenantContext(principal), principal!.subject, id, body.reason) });
    } catch (error) {
