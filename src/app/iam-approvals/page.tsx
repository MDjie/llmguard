...
export default function IamApprovalsPage(){
  ...
  return <div className="space-y-5 p-6"><div className="flex justify-between"><div><h1 className="text-2xl font-semibold">授权审批</h1><p className="mt-1 text-sm text-muted-foreground">审批与申请、目标账户相互独立。应急访问需要两名不同角色的审批人。</p></div><Button variant="outline" onClick={()=>void reload()}>刷新</Button></div>
    ...
    {rows.map(row=>{
      ...
      const allowed=row.status==='pending'&&row.requesterId!==userId&&row.targetUserId!==userId&&!row.decisions.some(item=>item.actorId===userId)&&can(recovery?'iam:recovery:approve':'iam:changes:approve');
