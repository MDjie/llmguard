...
export default function SecurityAlertsPage() {
  ...
  return <div className="space-y-4">
    ...
    {selected && <Card><CardContent className="space-y-4 pt-6"><div className="flex justify-between gap-3"><div><h2 className="font-semibold">{selected.riskId}</h2><p className="text-sm text-muted-foreground">{selected.stage} · {labels[selected.action]} · {stamp(selected.occurredAt)}</p></div><Button variant="ghost" onClick={() => { detailController.current?.abort(); setSelected(null); }}>关闭详情</Button></div>
