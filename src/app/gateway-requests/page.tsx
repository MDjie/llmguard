...
function stamp(value:string) { return new Intl.DateTimeFormat('zh-CN', { timeZone: 'Asia/Shanghai', dateStyle: 'short', timeStyle: 'medium' }).format(new Date(value)); }

export default function GatewayRequestsPage() {
  const listGeneration = useRef(0), selectedRef = useRef('');
  ...
  const refresh=useCallback(async(signal?:AbortSignal)=>{
    const generation = ++listGeneration.current; setLoading(true);setError('');
    try { const response=await fetch('/api/gateway/requests'+(state==='ALL'?'':'?state='+state),{signal}); if(!response.ok)throw new Error('无法读取执行记录，请确认登录权限及网关数据库迁移状态。');const result = gatewayRequestListSchema.parse(await response.json()); if (!signal?.aborted && generation === listGeneration.current) setList(result); }
    catch(error:unknown){if(!signal?.aborted)setError(error instanceof Error?error.message:'读取失败');}
    finally{if(!signal?.aborted)setLoading(false);}
  },[state]);
  useEffect(()=>{const controller=new AbortController();void refresh(controller.signal);return()=>controller.abort();},[refresh]);
  useEffect(()=>{
    selectedRef.current = selected; if(!selected)return; const controller=new AbortController();setDetail(null);setDetailError('');
  ...
  const loadMoreList = async () => {
    if (!list?.nextCursor) return; const generation = listGeneration.current; setMoreLoading(true);
    try { const response = await fetch('/api/gateway/requests?cursor=' + encodeURIComponent(list.nextCursor) + (state === 'ALL' ? '' : '&state=' + state));
      if (!response.ok) throw new Error('翻页失败，请刷新查询。'); const result = gatewayRequestListSchema.parse(await response.json());
      if (generation === listGeneration.current) setList(previous => previous ? { ...result, items: [...previous.items, ...result.items] } : result);
    } catch (failure: unknown) { if (generation === listGeneration.current) setError(failure instanceof Error ? failure.message : '翻页失败'); }
    finally { setMoreLoading(false); }
  };
  const loadMoreDetail = async () => {
    if (!detail?.pagination?.nextCursor) return; const id = selected; setMoreLoading(true);
    try { const response = await fetch('/api/gateway/requests?id=' + encodeURIComponent(id) + '&cursor=' + encodeURIComponent(detail.pagination.nextCursor));
      if (!response.ok) throw new Error('链路翻页失败，请重新打开详情。'); const result = gatewayRequestDetailSchema.parse(await response.json());
      if (selectedRef.current === id) setDetail(previous => previous ? { ...result, steps: [...previous.steps, ...result.steps], events: [...previous.events, ...result.events],
        pagination: result.pagination ? { ...result.pagination, eventGaps: [...(previous.pagination?.eventGaps ?? []), ...result.pagination.eventGaps] } : undefined } : result);
    } catch (failure: unknown) { if (selectedRef.current === id) setDetailError(failure instanceof Error ? failure.message : '链路翻页失败'); }
    finally { setMoreLoading(false); }
  };
  ...
  return <div className="space-y-5">
    ...
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
      <Card className="min-w-0 gap-0 py-0"><CardHeader className="flex flex-wrap items-center gap-3 border-b py-3 sm:flex-row"><CardTitle className="mr-auto">业务请求</CardTitle>
        ...
        {list?.hasMore && <div className="p-4"><Button variant="outline" disabled={moreLoading || loading} onClick={() => void loadMoreList()}>加载更多请求</Button></div>}
      ...
      <DetailPanel title="执行详情" open={Boolean(selected)} onClose={()=>setSelected('')}>
        {detailError?<p role="alert" className="p-4 text-sm text-red-600">{detailError}</p>:!detail?<p className="p-4 text-sm text-muted-foreground">正在读取…</p>:<div className="space-y-5 p-4">
          ...
          {Boolean(detail.pagination?.eventGaps.length) && <p role="alert" className="text-sm text-amber-700">链路存在缺失事件：{detail.pagination?.eventGaps.map(gap => `${gap.from}—${gap.to}`).join('、')}。这些区间不能认定为已完整还原。</p>}
          {detail.pagination?.hasMore && <Button variant="outline" disabled={moreLoading} onClick={() => void loadMoreDetail()}>加载后续步骤和事件</Button>}
