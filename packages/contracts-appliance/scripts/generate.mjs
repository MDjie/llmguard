...
function typescriptType(schema) {
  ...
  if (Array.isArray(schema.type)) return unionType(schema.type, (type) => typescriptType({ type }));
...
function renderTypescript() {
  const lines = [
    ...
    "export const APPLIANCE_CONTRACT_SOURCE_SHA256 = '" + sourceHash + "' as const;",
    '',
  ];
  for (const [name, schema] of Object.entries(definitions)) {
    ...
    for (const [propertyName, propertySchema] of Object.entries(schema.properties ?? {})) {
      lines.push(
        '  readonly ' + propertyName + (required.has(propertyName) ? '' : '?') +
          ': ' + typescriptType(propertySchema) + ';',
      );
    }
    lines.push('}', '');
  }
...
function javaType(schema) {
  const reference = refName(schema);
  if (reference) return reference;
  if (schema.oneOf || Array.isArray(schema.type)) return 'Object';
...
function renderJava() {
  ...
  for (const [name, schema] of Object.entries(definitions)) {
    ...
    properties.forEach(([propertyName, propertySchema], index) => {
      ...
      lines.push('    ' + type + ' ' + propertyName + (index === properties.length - 1 ? '' : ','));
    });
    lines.push('  ) {}', '');
  }
  lines.push('}', '');
...
function pythonType(schema) {
  ...
  if (Array.isArray(schema.type)) return unionType(schema.type, (type) => pythonType({ type }));
...
function renderPython() {
  const lines = [
    ...
    "APPLIANCE_CONTRACT_SOURCE_SHA256 = '" + sourceHash + "'",
    '',
  ];
  for (const [name, schema] of Object.entries(definitions)) {
    if (schema.enum) {
    ...
    for (const [propertyName, propertySchema] of properties) {
      ...
      lines.push('    ' + propertyName + ': ' +
        (required.has(propertyName) ? type : 'NotRequired[' + type + ']'));
    }
    lines.push('');
  }
  return lines.join('\n').trimEnd() + '\n';
...
function renderGo() {
  const lines = [
    ...
    'const ApplianceContractSourceSHA256 = "' + sourceHash + '"',
    '',
  ];
  for (const [name, schema] of Object.entries(definitions)) {
    if (schema.enum) {
      ...
      for (const value of schema.enum) {
        const constantName = snakeCase(value).split('_').map(goName).join('');
...
function renderProto() {
  const lines = [
    ...
    'syntax = "proto3";',
    '',
    'package guardllm.appliance.contracts.v1;',
    '',
    'import "google/protobuf/struct.proto";',
    '',
    'option java_package = "io.guardllm.appliance.contracts.v1.proto";',
  ...
  for (const [name, schema] of Object.entries(definitions)) {
    ...
    Object.entries(schema.properties ?? {}).forEach(([propertyName, propertySchema], index) => {
      ...
      lines.push(
        '  ' + qualifier + protoType(itemSchema) + ' ' + snakeCase(propertyName) +
          ' = ' + (index + 1) + ';',
      );
    });
    lines.push('}', '');
...
function compatibilitySignature() {
  ...
  return { contractVersion: '1.0', definitions: signature, services: grpcServices };
}

const outputMap = {
  'generated/typescript/appliance-v1.ts': renderTypescript(),
  'generated/java/io/guardllm/appliance/contracts/v1/ApplianceContracts.java': renderJava(),
  'generated/python/appliance_contracts_v1.py': renderPython(),
  'generated/go/appliancev1/contracts.go': renderGo(),
  'proto/appliance/v1/appliance.proto': renderProto(),
};

const manifest = {
  contractVersion: '1.0',
  source: 'model/appliance-v1.schema.json',
  sourceSha256: sourceHash,
  outputs: Object.fromEntries(Object.entries(outputMap).map(([path, content]) => [
    path,
...
if (acceptBaseline) {
  const baselinePath = join(packageRoot, 'baseline', 'appliance-v1.compatibility.json');
  mkdirSync(dirname(baselinePath), { recursive: true });
  writeFileSync(baselinePath, JSON.stringify(compatibilitySignature(), null, 2) + '\n', 'utf8');
}

if (drift.length > 0) {
  process.stderr.write('Generated appliance contract drift: ' + drift.join(', ') + '\n');
  process.exitCode = 1;
} else {
  process.stdout.write(
    (checkMode ? 'Appliance contract outputs are current' : 'Generated appliance contract outputs') +
      ' (source ' + sourceHash + ')\n',
  );
}
