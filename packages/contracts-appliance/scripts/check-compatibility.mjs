...
const source = JSON.parse(
  readFileSync(join(packageRoot, 'model', 'appliance-v1.schema.json'), 'utf8'),
);
const baseline = JSON.parse(
  readFileSync(join(packageRoot, 'baseline', 'appliance-v1.compatibility.json'), 'utf8'),
);
...
export function findApplianceCompatibilityViolations(currentSource, acceptedBaseline) {
  const violations = [];
  for (const [definitionName, previous] of Object.entries(acceptedBaseline.definitions)) {
    ...
    for (const [propertyName, previousSchema] of Object.entries(previous.properties)) {
      ...
      } else if (!propertySchemaIsBackwardCompatible(previousSchema, currentSchema)) {
...
if (import.meta.main) {
  const violations = findApplianceCompatibilityViolations(source, baseline);
  if (violations.length > 0) {
    process.stderr.write('Breaking appliance v1 contract change:\n- ' + violations.join('\n- ') + '\n');
    process.exitCode = 1;
  } else {
    process.stdout.write('Appliance v1 is backward compatible with its accepted baseline\n');
  }
}
